...
int saferead(fd,buf,len) int fd;
char *buf;
int len;
{
	int r;
...
int safewrite(fd,buf,len) int fd;
char *buf;
int len;
{
	int r;
...
int fd;
{
	...
	if (flagneedrset)
	{
		...
		if (!flagpipelining)
		{
			substdio_flush(&ss7);
			code = smtpcode(0);
			if (code >= 400) die_proto(); /* rejected RSET? tsk */
		}
	}
	...
	if (flagpipelining)
	{
		...
		{
			result(code);
			if (smtpcode(0) < 400) die_proto(); /* rejected MAIL, accepted RCPT */
			if (smtpcode(0) < 400) die_proto(); /* why does the spec allow this? */
			return;
		}
		code = smtpcode(0);
		if (code >= 400)
